...
function doSearch(service, type, term)
{
  ...
  if (term.indexOf(':') > -1) {
    ...
    if (idPos > -1) {
      nextPos = (idPos < artistPos)?idPos:artistPos; 
      itemId = term.substring(idPos+3,(idPos < nextPos)?nextPos:term.length);
    }
  ...
  return getAccountId(player, service)
    ...
    .then((resList) => {
      if (serviceDef.isList && serviceDef.isList(resList, type)) {
    	  var items = serviceDef.list(resList, type);
    	  if (items.length != 0) {
    		  return Promise.resolve({ "status": "interact", "message": 'I have found ' + items.length + " " + type + "s. Would you like me to play %s or hear the next choice?", "list": items, "playURI": service + '/' + type + '/id:%s' } );
    	  }
    	  return Promise.reject({ "status": "error", "message": "I couldn't find anything" } );
      }
