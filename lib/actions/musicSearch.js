...
function doSearch(service, type, term)
{
  ...
  if (term.indexOf(':') > -1) {
    ...
    //idPos should be without other qualifiers
    if (idPos > -1) {
        itemId = term.substring(idPos+3,term.length);
    }
  ...
  if ((serviceDef.country != '') && (country == '')) {
    ...
    if ( type == 'playlist_direct' ) {
	  ...
	  console.log(serviceDef.headers());
	  
    return request({url: url, json: true, headers: serviceDef.headers() });
  }
}
