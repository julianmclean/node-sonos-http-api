...
const deezerDef = {
  ...
  search:    {
               ...
               playlist: 'https://api.deezer.com/search/playlist?limit=|limit|&index=|index|&q='
             },
  metastart: {
               album:   '0004006calbum-',
               song:    '00032020tr%3a',
  ...
  playlistUri: 'https://api.deezer.com/playlist/',
  list:		 list,
  isList:    isList,
  buildUrl:  buildUrl,
}  
...
function loadTracks(type, tracksJson)
{
  ...
  var trackList = '';
  if ( type == 'playlist' ) {
	  trackList = tracksJson.tracks.data;
	  tracks.isPlaylist = true; //Forces a Queue Clear
  } else {
	  trackList = tracksJson.data;
  }
  // Load the tracks from the json results data
  if (trackList.length > 0) {
    // Filtered list of tracks to play
    tracks.queueTracks = trackList.reduce(function(tracksArray, track) {
      var skip = false;
  
      for (var j=0; (j < tracksArray.length) && !skip ; j++) {
        // Skip duplicate songs
        skip = (track.title == tracksArray[j].trackName && track.artist.name == tracksArray[j].artistName);
      }
...
{
  return type == 'playlist' && !resList.id;
}
...
{
  if ( type == 'playlist' ) {
	  return ( resList.tracks.data.length == 0 );
  }
